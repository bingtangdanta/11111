...
#: 默认重试次数（用户要求 3 次；可用 ASHARE_RETRIES 调）
try:
    RETRIES = int(_os.environ.get("ASHARE_RETRIES", "3"))
except ValueError:
    RETRIES = 3

#: 请求间隔：**默认每次随机 1~2 秒**（用户要求）。
#: 为什么从固定 0.35s 改大：GitHub Actions 是海外 IP，东财对海外请求拦得很凶，
#: 打太快会成片 502 / RemoteDisconnected。慢一点反而总耗时更短（少了大量重试）。
try:
    MIN_INTERVAL = float(_os.environ.get("ASHARE_MIN_INTERVAL", "0"))
except ValueError:
    MIN_INTERVAL = 0.0
try:
    _lo, _hi = (float(x) for x in _os.environ.get("ASHARE_MIN_INTERVAL_RANGE", "1,2").split(","))
    MIN_INTERVAL_RANGE = (min(_lo, _hi), max(_lo, _hi))
except (ValueError, TypeError):
    MIN_INTERVAL_RANGE = (1.0, 2.0)
if MIN_INTERVAL > 0:                     # 显式设了固定值就用固定值
    MIN_INTERVAL_RANGE = (MIN_INTERVAL, MIN_INTERVAL)
_last_call = 0.0
...
def throttle(min_interval: float | None = None) -> None:
    """
    请求间节流：默认**每次随机等 1~2 秒**（海外 IP 必须的礼貌，也是防 502 的第一道闸）。

    传了 min_interval 就用固定值（例如重试时多等一会儿）。
    """
    global _last_call
    if min_interval is not None:
        base = float(min_interval)
    else:
        base = _random.uniform(*MIN_INTERVAL_RANGE)
    wait = base - (time.monotonic() - _last_call)
    if wait > 0:
        time.sleep(wait)
...
#: 服务器瞬时故障 / 被反爬拦一下：这些错误**值得重试**（等几秒往往就好了）
RETRY_STATUS = (500, 502, 503, 504, 520, 521, 522, 524, 429)


def is_retryable(exc: Exception) -> bool:
    """这个异常值不值得重试？（502/503/504/429、连接被断、超时都算）"""
    import http.client
    import urllib.error
    ...
    if isinstance(exc, (urllib.error.URLError, TimeoutError, ConnectionError,
                        ...
                        http.client.BadStatusLine, OSError)):
        return True
    msg = str(exc).lower()
    return any(k in msg for k in ("502", "503", "504", "bad gateway", "timed out",
                                  "reset by peer", "remote end closed", "connection aborted",
                                  "remotedisconnected", "connection reset"))


def retry_sleep(attempt: int) -> float:
    """重试前的等待：2~4 秒（带抖动）。第 1 次重试约 2s，第 2 次约 3s，第 3 次约 4s。"""
    base = min(4.0, 2.0 + attempt)
    return base + _random.random()


def get_json(url: str, params: dict | None = None, *, hosts: list[str] | None = None,
             path: str = "", retries: int = RETRIES, timeout: int = 25,
